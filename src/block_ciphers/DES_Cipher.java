...
public class DES_Cipher {
	...
	private static final byte[] IPINV = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7,
	...
	private static final byte[] PC2 = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21,
			10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47,
			55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36,
			29, 32 };

	private static final byte[] E_BOX = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9,
	...
	public static void main(String[] args) {
		...
		case "CBC":
			...
			break;
		default:
			System.err.println("Invalid settings: 'Mode'\n"
					+ "Valid options are: ECB|CBC\n"
					+ "Check configuration file");
			throw new AssertionError();
		}
	 ...
	 * @param table
	 *            - a hard-coded table or list specifying which bits should be
	 ...
	 * @param d_0
	 *            - the highest 28 bits corresponding to the original key.
	 */
	private static void generateKeys(int c_0, int d_0) {
		...
		for (int i = 1; i <= subKeys.length; i++) {
			...
			c_next = leftRotate(c_prev, v); // debug28BitsWord(c_next, i, "C",
			// 2);
	 ...
	 * @param idx
	 *            - an index (For reference only).
	 * @param name
	 *            - a name (For reference only).
	 * @param radix
	 *            - can be either 2 for binary or 16 for hexadecimal.
	 */
	private static void debug28BitsWord(int bitsWord, int idx, String name,
			int radix) {
		if (radix == 2) {
	 ...
	 *            - can be either 2 for binary or 16 for hexadecimal.
	 */
	private static void debug48BitsWord(long bitsWord, int idx, String name,
			int radix) {
		if (radix == 2) {
	 ...
	 *            - a 32bit word.
	 * @param key
	 *            - a 48bit word, containing the current key.
	 *            <p>
